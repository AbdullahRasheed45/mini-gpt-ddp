"""
A GPT implemented from scratch, sized for training on Kaggle T4s.

Design notes (T4-specific):
...
- No bf16 anywhere: T4 supports fp16 but not bf16. Mixed precision is
  handled in train.py with GradScaler; the model itself stays dtype-agnostic.
"""
...
class GPTConfig:
    ...
    n_layer: int = 8
    n_head: int = 8
    n_embd: int = 512                # ~38M params with the defaults
...
class GPT(nn.Module):
    ...
    def generate(self, idx: torch.Tensor, max_new_tokens: int,
        ...
        """Naive sampling loop (no KV cache -- that's Project B's job)."""
        self.eval()
